...
class RedliningValidator:
    ...
    def validate(self):
        ...
        # First, check if there are any tracked changes by Agic to validate
        try:
            ...
            # Check for w:del or w:ins tags authored by Agic
            del_elements = root.findall(".//w:del", self.namespaces)
            ins_elements = root.findall(".//w:ins", self.namespaces)

            # Filter to only include changes by Agic
            agic_del_elements = [
                elem
                for elem in del_elements
                if elem.get(f"{{{self.namespaces['w']}}}author") == "Agic"
            ]
            agic_ins_elements = [
                elem
            ...
            # Redlining validation is only needed if tracked changes by Agic have been used.
            if not agic_del_elements and not agic_ins_elements:
                if self.verbose:
                    print("PASSED - No tracked changes by Agic found.")
                return True
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Remove Agic's tracked changes from both documents
            self._remove_agic_tracked_changes(original_root)
            self._remove_agic_tracked_changes(modified_root)
            ...
            if self.verbose:
                print("PASSED - All changes by Agic are properly tracked")
            return True

    def _generate_detailed_diff(self, original_text, modified_text):
        """Generate detailed word-level differences using git word diff."""
        error_parts = [
            "FAILED - Document text doesn't match after removing Agic's tracked changes",
            "",
            "Likely causes:",
    ...
    def _remove_agic_tracked_changes(self, root):
        """Remove tracked changes authored by Agic from the XML root."""
        ...
        for parent in root.iter():
            to_remove = []
            for child in parent:
                if child.tag == ins_tag and child.get(author_attr) == "Agic":
                    to_remove.append(child)
            for elem in to_remove:
                parent.remove(elem)

        # Unwrap content in w:del elements where author is "Agic"
        ...
        for parent in root.iter():
            to_process = []
            for child in parent:
                if child.tag == del_tag and child.get(author_attr) == "Agic":
